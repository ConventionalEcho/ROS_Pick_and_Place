...
void OpenManipulatorPickandPlace::demoSequence()
{
  ...
  {
  case 0: // home pose
    joint_angle.push_back( 0.00);	//Joint angle is array van angles van alles joints
    joint_angle.push_back(-1.05);
    joint_angle.push_back( 0.35);
    joint_angle.push_back( 0.70);
    ...
    demo_count_ ++;
    break;
  case 1: // initial pose	(Move a bit to see tags better)
    joint_angle.push_back( 0.01);
    joint_angle.push_back(-0.99);
    joint_angle.push_back( 0.00);
  ...
  case 2: // wait & open the gripper
    setJointSpacePath(joint_name_, present_joint_angle_, 3.0);	//Wait 3 seconds
    gripper_value.push_back(0.010);	//Add "open" position to gripper position array
    setToolControl(gripper_value);	//Set gripper position to position array
    demo_count_ ++;
    break;
  case 3: // pick the box
    for(int i = 0; i < ar_marker_pose.size(); i ++)
    {
      ...
      {										//kinematics_position is array of positions the arm should go to
        kinematics_position.push_back(ar_marker_pose.at(i).position[0]);	//ar_marker_pose gives location of ar markers [0] is X [1] is Y [2] is Z
        kinematics_position.push_back(ar_marker_pose.at(i).position[1]);
        kinematics_position.push_back(0.005);					//Pickup height is static
        kinematics_orientation.push_back(0.74);
        kinematics_orientation.push_back(0.00);
        kinematics_orientation.push_back(0.66);
        kinematics_orientation.push_back(-0.002);
    ...
    break;
  case 4: // wait & grip
    setJointSpacePath(joint_name_, present_joint_angle_, 1.0);		//Go to current position in 1 second (Just wait)
    gripper_value.push_back(-0.002);		//Gripper -0.002 is closed
    setToolControl(gripper_value);		//Set gripper position
    demo_count_ ++;
    break;
  case 5: // initial pose	(Just a move inbetween)
    joint_angle.push_back( 0.01);
    ...
    demo_count_ ++;
    break;
  case 6: // place pose		(Where it goes before placing block) Moet je hoger maken anders stoot hij de stapel om
    joint_angle.push_back( 1.57);
    joint_angle.push_back( 0.00);
    joint_angle.push_back(-0.79);
    joint_angle.push_back( 1.89);
    setJointSpacePath(joint_name_, joint_angle, 1.0);
    demo_count_ ++;
    break;
  case 7: // place the box
    kinematics_position.push_back(0);	//Block place position X
    kinematics_position.push_back(0.12);	//Block place position Y
    if(pick_ar_id_ == 0)  kinematics_position.push_back(0.091);		//Changed from relative to absolute position
    else if(pick_ar_id_ == 1)  kinematics_position.push_back(0.118);
    else if(pick_ar_id_ == 2)  kinematics_position.push_back(0.143);
    kinematics_orientation.push_back(0.74);
    kinematics_orientation.push_back(0.00);
    kinematics_orientation.push_back(0.66);
    kinematics_orientation.push_back(0.00);
    setTaskSpacePath(kinematics_position, kinematics_orientation, 1.5);
    demo_count_ ++;
    break;
  case 8: // wait & place
    setJointSpacePath(joint_name_, present_joint_angle_, 1.0);	//Wait 1 second
    gripper_value.push_back(0.010);	//Set gripper open position
    setToolControl(gripper_value);	//Move gripper
    demo_count_ ++;
    break;
